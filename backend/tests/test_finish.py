...
def _close_all_gates(client, sid):
    ...
    for stage in ("tutorial", "instruction", "modeling"):
        ...
        client.post(
            ...
            json={"phase": stage, "timepoint": "post", "function_class": "baseline", "pleasure": 0, "arousal": 0, "dominance": 0, "enjoyment": 1, "confusion": 1, "frustration": 1, "boredom": 1},
        )
    for loop in range(1, 7):
        client.post(f"/sessions/{sid}/sync/kid-response-complete", json={"loop_index": loop})
        client.post(
            f"/sessions/{sid}/self-reports",
            json={"loop_index": loop, "phase": "rehearsal", "timepoint": "pre", "function_class": "not_applicable", "pleasure": 0, "arousal": 0, "dominance": 0, "enjoyment": 1, "confusion": 1, "frustration": 1, "boredom": 1},
        )
        client.post(f"/sessions/{sid}/sync/feedback-delivered", json={"loop_index": loop})
        client.post(
            f"/sessions/{sid}/self-reports",
            json={"loop_index": loop, "phase": "feedback", "timepoint": "post", "function_class": "not_applicable", "pleasure": 0, "arousal": 0, "dominance": 0, "enjoyment": 1, "confusion": 1, "frustration": 1, "boredom": 1},
        )
...
def test_collected_even_when_gate_still_open(client, protocol_id):
    ...
    client.post(
        f"/sessions/{sid}/self-reports",
        json={"loop_index": 3, "phase": "rehearsal", "timepoint": "pre", "function_class": "not_applicable", "pleasure": 0, "arousal": 0, "dominance": 0, "enjoyment": 1, "confusion": 1, "frustration": 1, "boredom": 1},
    )
